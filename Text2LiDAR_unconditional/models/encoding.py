...
class FourierFeatures(nn.Module):
    ...
    def forward(self, coords):
        ...
        # out = coords.sin()[:, 0:1, :, :].cpu().detach().numpy()
        # out = out.squeeze()
        # plt.xticks([])
        # plt.yticks([])
        # plt.imshow(out, cmap='jet')
        # plt.savefig('/project/r2dm-main/debugfig/coordsin.png', dpi=300, bbox_inches='tight', pad_inches=0)
        # out = coords.cos()[:, 0:1, :, :].cpu().detach().numpy()
        # out = out.squeeze()
        # plt.xticks([])
        # plt.yticks([])
        # plt.imshow(out, cmap='jet')
        # plt.savefig('/project/r2dm-main/debugfig/coordcos.png', dpi=300, bbox_inches='tight', pad_inches=0)
        return encoded
