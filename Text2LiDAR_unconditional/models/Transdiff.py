...
class Transdiff(nn.Module):
    ...
    def __init__(
        ...
        self.decoder = Decoder(embed_dim=384, token_dim=64, depth=2, img_size=[64, 1024])
        # spatial coords embedding
        coords = encoding.generate_polar_coords(*self.resolution)
        self.in_conv = ops.Conv2d(32 + 2, 64, 3, 1, 1, ring=ring)
        ...
        if coords_embedding == "spherical_harmonics":
            ...
            in_channels += self.coords_embedding.extra_ch
        # self.coords_transfer = ops.Conv2d(32, 1, 3, 1, 1, ring=ring)
        # timestep embedding
    ...
    def forward(self, images: torch.Tensor, timesteps: torch.Tensor) -> torch.Tensor:
        ...
        h = self.in_conv(h)
        h_1_16, h_1_8, h_1_4, h_1_2 = self.rgb_backbone(h, temb) # h_1_16=B,4*64,384  # h_1_8=B,8*128,64  # h_1_4=B,16*256,64
        h_1_16 = self.transformer(h_1_16) # B, 256, 384
        saliency_h_1_16, h_1_16 = self.token_trans(h_1_16, temb)
        output, output_multi = self.decoder(saliency_h_1_16, h_1_16, h_1_8, h_1_4, h_1_2, temb)
        return output, output_multi
    
        # original spatial setting
        # # spatial embedding
        # if self.coords_embedding is not None:
        #     cemb = self.coords_embedding(self.coords)
        #     cemb = cemb.repeat_interleave(h.shape[0], dim=0) # B, 32, 64, 1024
