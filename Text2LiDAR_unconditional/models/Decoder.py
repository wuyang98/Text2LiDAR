...
class token_trans(nn.Module):
    ...
    def forward(self, fea, temb):
        B, _, _ = fea.shape
        fea = self.mlp(self.norm(fea)) # 1,1024,384

        # fea = torch.cat((saliency_tokens, fea), dim=1) # 1,1025,384
        fea = torch.cat((fea, temb), dim=1) # 1,1026,384

        fea = self.encoderlayer(fea) # 1,1026,384
        # saliency_tokens = fea[:, 0, :].unsqueeze(1) # 1,1,384
        # fea = fea[:,1:-1,:]

        saliency_fea = self.saliency_token_pre(fea) # 1,1024,384

        # reproject back to 64 dim
        saliency_fea = self.mlp2(self.norm2(saliency_fea)) # 1,1024,64

        return saliency_fea, fea


class decoder_module(nn.Module):
    def __init__(self, dim=384, token_dim=64, img_size=[64, 1024], ratio=8, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), fuse=True):
...
class Decoder(nn.Module):
    def __init__(self, embed_dim=384, token_dim=64, depth=2, img_size=[64, 1024]):
        ...
        self.avg = nn.AdaptiveAvgPool2d(output_size=(32, 512))
        self.sig = nn.Sigmoid()
    ...
    def forward(self, saliency_h_1_16, h_1_16, h_1_8, h_1_4, h_1_2, temb):
        device = temb.device
        # conv = nn.Conv2d(2, 2, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1)).to(device)
        B, _, _, = h_1_16.size()
        temb = temb.unsqueeze(dim=1) # B,1,384
        saliency_h_1_16 = self.mlp(self.norm(saliency_h_1_16))
        # saliency_fea_1_16 B,256,64
        mask_1_16 = self.pre_1_16(saliency_h_1_16) # B,256,2
        mask_1_16 = mask_1_16.transpose(1, 2).reshape(B, 2, self.img_size[0] // 16, self.img_size[1] // 16) # B,2,4,64

        # 1/16 -> 1/8
        fea_1_8 = self.decoder1(h_1_16[:, 0:-1, :], h_1_8) # B,1024,64
        saliency_fea_1_8, token_fea_1_8 = self.token_pre_1_8(fea_1_8, temb)
        mask_1_8 = self.pre_1_8(saliency_fea_1_8)
        mask_1_8 = mask_1_8.transpose(1, 2).reshape(B, 2, self.img_size[0] // 8, self.img_size[1] // 8) # B,2,8,128

        # 1/8 -> 1/4
        fea_1_4 = self.decoder2(token_fea_1_8[:, 0:-1, :], h_1_4) # B,4096,64
        saliency_fea_1_4, token_fea_1_4 = self.token_pre_1_4(fea_1_4, temb)
        mask_1_4 = self.pre_1_4(saliency_fea_1_4)
        mask_1_4 = mask_1_4.transpose(1, 2).reshape(B, 2, self.img_size[0] // 4, self.img_size[1] // 4) # B,2,16,256

        # 1/4 -> 1/2
        fea_1_2 = self.decoder3(token_fea_1_4[:, 0:-1, :], h_1_2) # B,4096,64
        saliency_fea_1_2, token_fea_1_2 = self.token_pre_1_2(fea_1_2, temb)
