...
class Token_performer(nn.Module):
    def __init__(self, dim, in_dim, head_cnt=1, kernel_ratio=0.5, dp1=0.1, dp2 = 0.1):
        ...
        # self.bi = torch.randn(self.m, self.emb)
        # self.w = nn.Parameter(nn.init.orthogonal_(self.bi) * math.sqrt(self.m), requires_grad=False)
    ...
    def prm_exp(self, x):
        ...
        # SM(x, y) = E_w[exp(w^T x - |x|/2) exp(w^T y - |y|/2)]
        # therefore return exp(w^Tx - |x|/2)/sqrt(m)
        bi = torch.randn(self.m, self.emb).to(x.device)
        w = nn.Parameter(nn.init.orthogonal_(bi) * math.sqrt(self.m), requires_grad=False).to(x.device)
        xd = ((x * x).sum(dim=-1, keepdim=True)).repeat(1, 1, self.m) / 2
        wtx = torch.einsum('bti,mi->btm', x.float(), w)
