...
class saliency_token_inference(nn.Module):
    ...
    def forward(self, fea):
        B, N, C = fea.shape
        x = self.norm(fea)
        # T_s, F_s = x[:, 0, :].unsqueeze(1), x[:, 0:-2, :]
        F_s = x[:, 0:-2, :]
        T_s_0 = x[:, -1, :].unsqueeze(1)
        T_s_1 = x[:, -2, :].unsqueeze(1)

        q = self.q(F_s).reshape(B, N-2, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        k = self.k(T_s_0).reshape(B, 1, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        v = self.v(T_s_1).reshape(B, 1, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
...
class token_Transformer(nn.Module):
    ...
    def forward(self, h_1_16, temb, text_emb):
        B, _, _ = h_1_16.shape
        ...
        h_1_16 = torch.cat((h_1_16, text_emb), dim=1) # B,259,384

        h_1_16 = self.encoderlayer(h_1_16) # B,259,384
