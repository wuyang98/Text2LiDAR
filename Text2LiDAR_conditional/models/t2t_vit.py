...
def _cfg(url='', **kwargs):
    return {
        'url': url,
        'num_classes': 1000, 'input_size': (2, 32, 1024), 'pool_size': None,
...
class T2T_module(nn.Module):
    ...
    """
    def __init__(self, img_size=[32, 1024], tokens_type='performer', in_chans=3, embed_dim=768, token_dim=64):
        super().__init__()
        ...
        if tokens_type == 'transformer':
            ...
            self.attention1 = Token_transformer(dim=in_chans * 64 * 3, in_dim=token_dim, num_heads=1, mlp_ratio=1.0)
            self.attention2 = Token_transformer(dim=token_dim * 3 * 3, in_dim=token_dim, num_heads=1, mlp_ratio=1.0)
            self.attention3 = Token_transformer(dim=token_dim * 3 * 3, in_dim=token_dim, num_heads=1, mlp_ratio=1.0)
            self.project = nn.Linear(token_dim * 3 * 3, embed_dim)

            # self.trans_1_4 = ops.Conv2d(2050, 2116, 3, 1, 1, ring=True)
    ...
    def forward(self, x):
        # step0: soft split
        H, W = 32, 1024
        x = self.soft_split0(x).transpose(1, 2) # B,34,64,1024----B,16384,306
        x_1_2 = self.attention1(x) # B,16384,64
        B, _, C = x_1_2.shape
        x = x_1_2.transpose(1,2).reshape(B, C, int(H/2), int(W/2)) # B,64,32,512

        x = self.soft_split1(x).transpose(1, 2) # B,64,32,512----B,4096,576
        x_1_4 = self.attention2(x) # B,4096,64
        B, _, C = x_1_4.shape
        x = x_1_4.transpose(1,2).reshape(B, C, int(H/4), int(W/4)) # B,64,16,256

        x = self.soft_split2(x).transpose(1, 2) # B,64,16,256----B,1024,576
        x_1_8 = self.attention3(x) # B,1024,64
        B, _, C = x_1_8.shape
        x = x_1_8.transpose(1, 2).reshape(B, C, int(H/8), int(W/8)) # B,64,8,128
        x = self.soft_split3(x).transpose(1, 2) # B,64,8,128----B,256,576

        # final tokens
        x = self.project(x) # B,256,384

        return x, x_1_8, x_1_4, x_1_2

def pose_linear(pos):
    pose_linear =  nn.Linear(in_features=257, out_features=197, bias=True).to(pos.device)
    pos = einops.rearrange(pos, "B H W -> B W H")
    pos = pose_linear(pos)
    pos = einops.rearrange(pos, "B W H -> B H W")
    return pos
def x_linear(x):
    x_linear =  nn.Linear(in_features=197, out_features=257, bias=True).to(x.device)
    x = einops.rearrange(x, "B H W -> B W H")
    x = x_linear(x)
    x = einops.rearrange(x, "B W H -> B H W")
    return x

class T2T_ViT(nn.Module):
    def __init__(self, img_size=[32, 1024], tokens_type='performer', in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.pos_embed = nn.Parameter(data=get_sinusoid_encoding(n_position=num_patches + 2, d_hid=embed_dim), requires_grad=True)
    ...
    def forward_features(self, x, temb):
        B = x.shape[0]
        # pos_embed = nn.Parameter(data=get_sinusoid_encoding(n_position=128 + 1, d_hid=384), requires_grad=True).to(x.device)
        x, x_1_8, x_1_4, x_1_2 = self.tokens_to_token(x)
        temb = temb.unsqueeze(1)
        cls_tokens = self.cls_token.expand(B, -1, -1)
        x = torch.cat((cls_tokens, x), dim=1) # B,129,384
        x = torch.cat((x, temb), dim=1) # B,257,384
        x = x + self.pos_embed
        x = self.pos_drop(x)
        ...
        x = self.norm(x)
        return x[:, 1:-1, :], x_1_8, x_1_4, x_1_2

    def forward(self, x, temb):
        x, x_1_8, x_1_4, x_1_2 = self.forward_features(x, temb)
        return x, x_1_8, x_1_4, x_1_2
...
def T2t_vit_t_14(pretrained=True, **kwargs):  # adopt transformers for tokens to token
    ...
    # model = T2T_ViT(tokens_type='transformer', embed_dim=384, depth=14, num_heads=6, mlp_ratio=3., img_size=[64, 1024])
    model.default_cfg = default_cfgs['T2t_vit_t_14']
    # args = kwargs['args']
    if pretrained:
        load_checkpoint(model, './pretrained_model/80.7_T2T_ViT_t_14.pth.tar', use_ema=True)
        # print('Model loaded from {}'.format('./pretrained_model/80.7_T2T_ViT_t_14.pth.tar'))
    return model
