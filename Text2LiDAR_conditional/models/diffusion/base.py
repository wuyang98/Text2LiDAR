...
class GaussianDiffusion(nn.Module):
    ...
    def p_loss(
        ...
        prediction, prediction_multi = self.denoiser(xt, condition, text)
    ...
    def forward(
        self, x_0: torch.Tensor, text: torch.Tensor, loss_mask: torch.Tensor | None = None
    ) -> torch.Tensor:
        # shared in continuous/discrete versions
        steps = self.sample_timesteps(x_0.shape[0], x_0.device)
        loss = self.p_loss(x_0, text, steps, loss_mask)
        return loss
